...
const getGroups = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: 'Error obteniendo los grupos' });
  }
};
...
const updateGroup = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: 'Error actualizando el grupo' });
  }
};
...
const deleteGroup = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: 'Error eliminando el grupo' });
  }
};
...
const getUserGroups = async (req, res) => {
  try {
    ...
    if (!groups.length) {
      return res.status(404).json({ success: false, message: 'No se encontraron grupos para este usuario' });
    }
    res.status(200).json({ success: true, data: groups });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: 'Error obteniendo los grupos del usuario' });
  }
};
